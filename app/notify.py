"""
Envoi des notifications push via ntfy.sh quand un signal est detecte.
"""
...
def send_signal_notification(signal: Signal) -> bool:
    emoji = "vert" if signal.direction == "achat" else "rouge"
    confidence_tag = " HAUTE CONFIANCE" if signal.high_confidence else ""
    mode_tag = "[SCALPING]" if signal.mode == "scalping" else "[PRUDENT]"

    title = f"{signal.asset} - {signal.direction.upper()} ({signal.timeframe}){confidence_tag}"
    message = (
        f"{mode_tag}\n"
        f"Entree: {signal.entry_price:.5f}\n"
    ...
    try:
        ...
        logger.error("Echec envoi notification ntfy pour %s: %s", signal.asset, exc)
        return False
